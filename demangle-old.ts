...
class MangledName {
  public static parse(str: string): {
    name: MangledName | null;
    consumed: number;
  } {
    const name = new MangledName(str);
    if (!name.isValid) {
      return { name: null, consumed: 0 };
    }
    return { name, consumed: name.consumed };
  }
...
class MangledType {
  public static parse(str: string): {
    arg: MangledType | null;
    consumed: number;
  } {
    const arg = new MangledType(str);
  ...
  private constructor(str: string, parent = Type.ERROR) {
    ...
    if (str[0] === "P") {
      str = str.substring(1);
      this.consumed += 1;
      this.child = new MangledType(str, Type.Pointer);
    ...
    if (str[0] === "R") {
      ...
      str = str.substring(1);
      this.consumed += 1;
      this.child = new MangledType(str, Type.Reference);
      if (this.child.type === Type.ERROR) {
        return;
      }
      this.consumed += this.child.consumed;
    ...
    if (str[0] === "O") {
      ...
      str = str.substring(1);
      this.consumed += 1;
      this.child = new MangledType(str, Type.RValueReference);
      if (this.child.type === Type.ERROR) {
        return;
      }
      this.consumed += this.child.consumed;
  ...
  private child: MangledType | null = null;
...
export function demangle(input: string): string {
  ...
  let argumentTypes: MangledType[] = [];
  while (symbol.length > 0) {
    const { arg, consumed } = MangledType.parse(symbol);
